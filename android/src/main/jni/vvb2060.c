...
char* nGetPropsHash() {
		...
		string[SHA512_CBLOCK] = 0;
		propHashString = string;
		return propHashString;
}
...
jint JNI_OnLoad(JavaVM *jvm, void *v __unused) {
    ...
    if ((clazz = (*env)->FindClass(env, "lab/neruno/magisk_detector/Native")) == NULL) {
        return JNI_ERR;
    }
